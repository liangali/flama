...
mfxStatus CVPPTest::CreateVPLSession() {
    ...
    DBG_LOGF("Created m_session with library: %s", (char *)descPath);
...
mfxStatus CVPPTest::ProcessingFrame(void *inputTexture, void *outputTexture) {
...
#else
    ID3D11Texture2D *pInputTex = (ID3D11Texture2D *)inputTexture;
    ID3D11Texture2D *pOutputTex = (ID3D11Texture2D *)outputTexture;
#endif
    ...
    frameProfiler.MarkStageBegin(prof::Stage::ImportFrameSurface);
#ifdef _WIN32
    if (m_pOpts->surfaceType == MFX_SURFACE_TYPE_D3D11_TEX2D) {
        ...
        surfaceForImport.texture2D = pInputTex;
        ...
        //VERIFY(pmfxVPPInSurface != nullptr, "ERROR: ImportFrameSurface! null pmfxVPPInSurface", sts);
        mfxSurfaceD3D11Tex2D surfaceOForImport = {};
        surfaceOForImport.SurfaceInterface.Header.SurfaceFlags  = m_pOpts->surfaceFlag;
        surfaceOForImport.SurfaceInterface.Header.SurfaceType = m_pOpts->surfaceType;
        surfaceOForImport.SurfaceInterface.Header.StructSize = sizeof(mfxSurfaceD3D11Tex2D);
        surfaceOForImport.texture2D = pOutputTex;
        sts = m_memoryInterface->ImportFrameSurface(
            m_memoryInterface,
                MFX_SURFACE_COMPONENT_VPP_OUTPUT,
                &surfaceOForImport.SurfaceInterface.Header,
                &pmfxVPPOutSurface);
        VERIFY(MFX_ERR_NONE == sts, "ERROR: ImportFrameSurface", sts);
         //VERIFY(pmfxVPPOutSurface != nullptr, "ERROR: ImportFrameSurface! null pmfxVPPOutSurface", sts);
    }
    ...
    frameProfiler.MarkStageEnd(prof::Stage::ImportFrameSurface);
    // vpp next frame
    ...
    switch (sts) {
            ...
            if (syncp) {
                do {
                    ...
                    if (m_pOpts->bNoSSA == true ||
                        ...
                        if (MFX_ERR_NONE == sts && IsDebugMode()) {
        ...
        default:
            ...
            if (sts < 0 && sts != MFX_ERR_MORE_DATA)
                std::cerr << "ERROR: MFXVideoVPP_RunFrameVPPAsync(), " << sts << '\n';
            break;
    }
...
mfxStatus CVPPTest::ProcessStreamVPP() {
    ...
    DBG_LOGF("VPP %u frames", frameNum);
