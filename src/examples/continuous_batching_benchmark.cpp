...
#include "continuous_batching_benchmark.h"
...
try
{
    ...
    CBPipeline::GenerationInfoCollector generation_info_collector;

    std::atomic<bool> finishGenerationThread{false};

    uint16_t request_id = 0;
    std::thread AddGenerationThread(CBPipeline::AddGeneration, &pipe, &dataset, request_id, &generation_info_collector, is_speculative_decoding_enabled);
    AddGenerationThread.join();

    std::thread lmmEngineThread(CBPipeline::llmEngineLoop, &pipe, &dataset, &finishGenerationThread);
    std::thread statisticsReporterThread(CBPipeline::statisticsReporter, &generation_info_collector, num_prompts);
